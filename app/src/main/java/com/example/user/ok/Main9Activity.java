...
public class Main9Activity extends AppCompatActivity {
    ...
    // EditText name;
    EditText phone;
    EditText email,branch,Date,quantity,cost,toy;
    Button reg;
    Connectionclass Connectionclass;
    String Z="";
    int c=0;
    String fg,p,n,dd,kj,e,oj;
    int toc,qua,cos;
    @Override
            ...
            if (phone.length() == 0 || email.length() == 0 || branch.length() == 0 || Date.length() == 0 || cost.length() == 0 || quantity.length() == 0||toy.length()==0) {
                ...
                try {
                    ...
                    if (con == null) {
                        ...
                        c++;
                        String q = "insert into computer_orders values(?,?,?,?,?,?,?,?)";
